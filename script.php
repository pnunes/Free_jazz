...
{	$file = $_FILES['photoupload']['tmp_name'];
	...
	$_UP['extensoes'] = array('txt', 'doc', 'csv', 'pdf','htm', 'html', 'zip', 'rar','tif','jpg','gif');
	...
	if (!is_uploaded_file($file) || ($_FILES['photoupload']['size'] > 16 * 1024 * 1024) )
	{
	  ...
	  $error = 'Faça upload de arquivos menores que 16Mb!!!';
		...
		$aux_tipo_imagem = $size['mime'];
			...
			move_uploaded_file($_FILES['photoupload']['tmp_name'], 'hawbs/'.$_FILES['photoupload']['name']);
		    chmod('hawbs/'.$_FILES['photoupload']['name'], 0777);
