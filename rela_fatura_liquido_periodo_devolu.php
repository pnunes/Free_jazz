  ...
  session_start();
    ...
    switch (get_post_action('gera')) {
         ...
         case 'gera':
              ...
              $conta = "SELECT sum(qtdade) AS tot_qtd
              FROM remessa
              WHERE ((dt_envio>='$v_dt_inicio')
              ...
              ///////////////////////GERA O TOTAL LIQUIDO EM VALOR DAS HAWBS ENTREGUES //////////////////////////////////

              $soma_0 = "SELECT sum((remessa.qtdade*remessa.valor)- (pessoa.c_servico*remessa.qtdade)) AS liq_fatu
              FROM remessa,pessoa
              WHERE ((dt_envio>='$v_dt_inicio')
              AND (dt_envio<='$v_dt_fim')
              AND (remessa.entregador=pessoa.matricula))";
              $liquido = mysql_db_query($banco_d,$soma_0,$con) or die ("Não foi possivel acessar o banco 2");
              $total_1 = mysql_num_rows($liquido);
              for($ic=0; $ic<$total_1; $ic++){
                  $row = mysql_fetch_row($liquido);
                  $v_soma_liqui          = $row[0];
              }
              $v_liquido = number_format($v_soma_liqui, 2, ",", ".");
              
              ///////////////////////GERA O TOTAL BRUTO EM VALOR DAS HAWBS ENTREGUES //////////////////////////////////
...
<HTML>
<HEAD>
 <TITLE>rela_fatura_liquido_periodo.php</TITLE>
</HEAD>
<BODY>
  <style>
  ...
  <div id="geral" align="center">
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="0" background="img/blue.jpg">
     <tr>
       ...
       <td width="50%">
         <p align="right"><font face="Arial" size="2" color="#FFFFFF"><b>Relatório Faturamento Período</b></font></td>
     </tr>
   </table>
   ...
   <table width="70%" border="1" cellpadding="5" cellspacing="0" bordercolor="#6495ED">
        <form name="cadastro" method="POST" action="rela_fatura_liquido_periodo_devolu.php" border="20">
          <tr>
