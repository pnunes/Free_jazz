  ...
  session_start();
    ...
    switch (get_post_action('gera')) {
         ...
         case 'gera':
              ...
              $contar = "SELECT count(n_hawb) AS tot_qtd FROM remessa
              WHERE ((codi_cli='$cliente')
              AND (escritorio='$escritorio')
              AND (dt_remessa>='$v_dt_inicio')
              AND (dt_remessa<='$v_dt_fim'))";
              $somar = mysql_db_query($banco_d,$contar,$con) or die ("Não foi possivel acessar o banco (soma)");
              $tot = mysql_num_rows($somar);

              for($ic=0; $ic<$tot; $ic++){
                  $row = mysql_fetch_row($somar);
                  $v_tota          = $row[0];
              }
              ////////////////////////////////////GERA TOTAL VALOR RELATORIO/////////////////////////////////////
              $soma = "SELECT co_servico,classe_cep,qtdade,valor
              FROM remessa
              WHERE ((codi_cli='$cliente')
              AND (escritorio='$escritorio')
              AND (dt_remessa>='$v_dt_inicio')
              ...
              $pega_esc = "SELECT nome FROM regi_dep WHERE codigo='$escritorio'";
              $query_2 = mysql_db_query($banco_d,$pega_esc,$con) or die ("Não foi possivel acessar o banco");
              ...
              $_SESSION['nome_escri_m'] =$nome_escri;
              ...
              require_once("gera_detalhe_fatu_cliente_escri.php");
...
<BODY>
  ...
  <div id="geral" align="center">
   ...
   <table width="70%" border="1" cellpadding="5" cellspacing="0" bordercolor="#6495ED">
        <form name="cadastro" method="POST" action="rela_detalhe_fatura_cliente_escri.php" border="20">
          <tr>
           <td colspan="2">
            <?php
