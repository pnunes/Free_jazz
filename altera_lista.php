  ...
  <div id="geral" align="center">
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="0" background="img/blue.jpg">
     <tr>
       ...
       <td width="50%">
         <p align="right"><font face="Arial" size="2" color="#FFFFFF"><b>Lista de Entrega - Alteração</b></font></td>
     </tr>
   </table>
   ...
   <form method="POST" action="altera_lista.php" border="20">
       ...
       <table width="500" heigth="300" align="center">
         <tr>
           <td>
              <?php
               echo "<center><Font size=\"2\" face=\"ARIAL\">Entregador Atual..:</font>";
               mysql_connect($base_d, $usuario_d, $senha_d) or die ("Erro de conexão");
               mysql_select_db($banco_d) or die ("Banco de dados inexistente");

               $resultado = mysql_query ("SELECT matricula,nome
               FROM pessoa ORDER BY nome");
               echo "<select name='entregador_atu' class='caixa' align=\"center\">\n";
               while($linha = mysql_fetch_row($resultado))  {
                  printf("<option value='$linha[0]'>$linha[0] - $linha[1] </option>\n");
               }
              ?>
       ...
       <table width="500" heigth="300" align="center">
         <tr>
           <td>
              <?php
               echo "<center><Font size=\"2\" face=\"ARIAL\">Novo entregador..:</font>";
               mysql_connect($base_d, $usuario_d, $senha_d) or die ("Erro de conexão");
               mysql_select_db($banco_d) or die ("Banco de dados inexistente");

               $resultado = mysql_query ("SELECT matricula,nome
               FROM pessoa ORDER BY nome");
               echo "<select name='entregador_novo' class='caixa' align=\"center\">\n";
               while($linha = mysql_fetch_row($resultado))  {
                  printf("<option value='$linha[0]'>$linha[0] - $linha[1] </option>\n");
               }
              ?>
   ...
   <?php
    ...
    case 'mostra':
           ...
           $entregador_atu  =$_POST['entregador_atu'];
           $entregador_novo =$_POST['entregador_novo'];
           
           $_SESSION['entregador_novo']   =$entregador_novo;
           
           //Pega o nome do entregador atual
           $verifi="SELECT nome FROM pessoa WHERE matricula='$entregador_atu'";
           ...
           //Pega o nome do entregador novo
           $verifi="SELECT nome FROM pessoa WHERE matricula='$entregador_novo'";
           $query = mysql_db_query($banco_d,$verifi,$con) or die ("Não foi possivel acessar o banco");
           $total = mysql_num_rows($query);
           ...
           <form name="cadastro" id="cadastro" action="altera_lista.php" method="post">

              <table width="60%" border="1" cellpadding="5" cellspacing="0" bordercolor="#6495ED">
                 <tr>
                    <td colspan="2" align="center"><font face="arial" size="2"><b>Entregadores/Remessa Selecionados :<font color="red">Atual:<?php echo "$nome_atu"; ?> - Novo: <?php echo "$nome_novo"; ?>/ <?php echo "$n_remessa"; ?></b></font></td>
                 </tr>
                 <?php
                    $con = mysql_connect($base_d, $usuario_d, $senha_d) or die ("Erro de conexão");
                    $res = mysql_select_db($banco_d) or die ("Banco de dados inexistente");

                    $sql = mysql_query("SELECT controle,cep_desti,nome_desti,rua_desti,bairro_desti
                    FROM remessa
                    WHERE ((n_remessa='$n_remessa')
                    AND (entregador='$entregador_atu'))");
                    ...
                    if (mysql_num_rows($sql)==0) {
                      ...
                      while ($x  = mysql_fetch_array($sql)) {
                         ...
                         echo $controle."<input type =\"checkbox\" name = \"lista[]\" id=\"lista\" value=\"$controle\">$cep_desti - $nome_desti - $rua_desti - $bairro_desti";
                         echo "</td></tr>";
                         echo "</div>";
                      }
                    }
      ...
      case 'grava':
             $entregador_novo  =$_SESSION['entregador_novo'];
             $dt_lista    =date('Y-m-d');
             $con = mysql_connect($base_d, $usuario_d, $senha_d) or die ("Erro de conexão");
             $res = mysql_select_db($banco_d) or die ("Banco de dados inexistente");
             $num=0;
             foreach($_POST['lista'] as $controle){
                ...
                $achou = mysql_num_rows($query);
                If ($achou > 0 ) {
                  mysql_query("UPDATE remessa SET entregador='$entregador_novo',dt_lista='$dt_lista'
