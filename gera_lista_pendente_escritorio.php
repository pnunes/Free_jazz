<?php
  ...
  function gera_lista_pendente_escritorio() {
     $escritorio        =$_SESSION['escritorio_m'];
     $nome_escritorio   =$_SESSION['nome_escritorio_m'];
     ...
     $pdf->SetName("RHAWB´s Pendentes de :$nome_escritorio - Período :$v_dt_inicio a $v_dt_fim");
     $pdf->SetCabecalho(" HAWB                     SERVIÇO                      DATA          NOME DESTINATÁRIO                                     RUA                                                                    BAIRRO                           CIDADE                      ENTREGADOR");
     $pdf->Open();
     $pdf->AddPage();
     $pdf->SetFont('Arial', '', 6);
     ...
     remessa.bairro_desti,remessa.cidade_desti,pessoa.nome
     FROM remessa,serv_ati,pessoa
     ...
     while ( $row = mysql_fetch_array($result) ) {
        ...
        $pdf->Cell(20, 5, $row[7], 0, 1);
     }
     $pdf->Output();
     }
     gera_lista_pendente_escritorio();
?>
