...
<HTML>
 <HEAD>
  <TITLE>Exclui_detalhes_entrega.php</TITLE>
...
<BODY>
  ...
  switch (get_post_action('exclui','mostra','seleciona')) {
         ...
         case 'seleciona':
             $controle      =$_POST['ocorrencia'];
             $_SESSION['controle_b']  =$controle;
             $localiza = "SELECT ocorrencia,date_format(dt_evento,'%d/%m/%Y')
     ...
     if (empty($codi_barra)) {
        ...
        <?php
         ...
         $verifi="SELECT n_hawb FROM controle_reentrega WHERE cod_barra='$codi_barra'";
         ...
         If ($total==0) {
            $codi_barra='';
            ?>
            <script language="javascript"> window.location.href=("exclui_detalhes_entrega.php")
 ...
 ?>
  <form name="cadastro" action="exclui_detalhes_entrega.php" method="post">
           ...
           <?php
              ...
              <?php
                ...
                while ( $linha = mysql_fetch_array($resul)) {
                    $select = $n_hawb == $linha[0] ? "selected" : "";
